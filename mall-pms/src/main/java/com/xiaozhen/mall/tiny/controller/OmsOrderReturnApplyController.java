...
public class OmsOrderReturnApplyController {
    ...
    @ResponseBody
    public CommontResult<OmsOrderReturnApply> getOrderReturnApply(@PathVariable("id")
                                                                  @ApiParam("id") Long id) {
        return CommontResult.success(returnApplyService.getOrderReturnApply(id));
    }

    @ApiOperation("批量删除申请")
    @RequestMapping(value = "/delete", method = RequestMethod.POST)
    @ResponseBody
    public CommontResult deleteOrderReturnApplyList(@RequestParam(name = "ids")
                                                    @ApiParam("ids") Long... ids) {
        CommontResult commontResult;
        int count = returnApplyService.deleteOrderReturnApplyList(ids);
        if (count == 1) {
            commontResult = CommontResult.success(ids);
            LOGGER.debug("deleteOrderReturnApplyList success:ids={}", ids);
        } else {
            commontResult = CommontResult.failed("操作失败");
            LOGGER.debug("deleteOrderReturnApplyList failed:ids={}", ids);
        }
    ...
    @ResponseBody
    public CommontResult<CommonPage<OmsOrderReturnApply>> listOrderReturnApply(@RequestParam(name = "createTime", defaultValue = "")
        ...
        List<OmsOrderReturnApply> returnApplyList = returnApplyService.listOrderReturnApply(createTime, handleMan,
